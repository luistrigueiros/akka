...
import akka.actor.{ ActorSystemSetting, ExtendedActorSystem }
...
object SerializationSettings {
   ...
   */
  def apply(createSerializers: ExtendedActorSystem ⇒ immutable.Seq[SerializerDetails]): SerializationSettings = {
    new SerializationSettings(createSerializers)
  }
   ...
   */
  def create(
    createSerializers: akka.japi.Function[ExtendedActorSystem, java.util.List[SerializerDetails]]): SerializationSettings =
...
object SerializerDetails {
   ...
   *               the classes, the alias and the serializer had been in the config.
   */
  def apply(alias: String, serializer: Serializer, useFor: java.util.List[Class[_]]): SerializerDetails =
    new SerializerDetails(alias, serializer, useFor.asScala.toVector)

}

/**
 * Internal API: Use the factories in [[SerializerDetails$]] to construct
 */
final class SerializerDetails(
  val alias:      String,
